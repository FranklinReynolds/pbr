...
def get_version(package_name, pre_version=None):
    ...
    if version:
        return version
    version = fetch_pkg_ver(package_name)  # FDR: hack workaround
    if version:                            #      hopefully unnecessary in
        return version                     #      a future release
    raise Exception("Versioning for this project requires either an sdist"
                    " tarball, or access to an upstream git repository."
                    " Are you sure that git is installed?")


def fetch_pkg_ver(package):
    ''' FDR: get the version number of the installed package that is actually
        being used. Start with sys.path for the list of directories that
        might have the package. Look for a matching file in each directory.
        If one is found, look for a corresponding directory that contains
        either a PKG-INFO or METADATA file. Otherwise return None.
    '''
    p_dir_list = sys.path
    for f_dir in p_dir_list:
        try:
            for f_file in os.listdir(f_dir):
                if f_file == package:
                    p_path = f_dir + "/" + package
                    ...
                    for fname in glob.glob(p_path + "-*"):
                        ...
                        for filename in pkg_metadata_filenames:
                            try:
                                pkg_metadata_file = open(fname + "/" + filename, 'r')
                            except (IOError, OSError):
                                continue
                            try:
                                pkg_metadata = email.message_from_file(pkg_metadata_file)
                                ...
                                continue
                        version = pkg_metadata.get('Version', None)
                        return version
        except OSError:
            continue
